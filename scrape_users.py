...
def scrape_users(cookies,headers,target,limit,type="following",proxies={},append=False,all_items=True):
    try:
        ...
        with open(f'accounts/accounts.json','r+',encoding='utf-8') as f:
            ...
            for user in entries:
                followers_count = 0
                if "result" in user["content"]["itemContent"]["user_results"].keys() and "legacy" in user["content"]["itemContent"]["user_results"]["result"].keys():
                    followers_count = user["content"]["itemContent"]["user_results"]["result"]["legacy"]["followers_count"]
                    
                else:
                    print(user["content"]["itemContent"]["user_results"])
                    continue
            ...
            print(f"scraped {len(entries)} accounts, saved {i} filtered accounts")
